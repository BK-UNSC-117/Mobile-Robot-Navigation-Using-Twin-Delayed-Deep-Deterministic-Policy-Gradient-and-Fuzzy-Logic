...
class turtle_sim_env(Node):
    def __init__(self):
        ...
        self.robot_x_init=np.random.uniform(low=6.5,high=8,size=1)[0]
        self.robot_y_init=np.random.uniform(low=2,high=8,size=1)[0]

        self.goal_x=np.random.uniform(low=2,high=4,size=1)[0]
        self.goal_y=np.random.uniform(low=4,high=6,size=1)[0]
